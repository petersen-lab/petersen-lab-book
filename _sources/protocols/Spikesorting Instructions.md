...
If you want to adjust Kilosort parameters, you can edit ```KilosortConfiguration.m``` file. You can also create a copy of this file, edit it, and move it to ```./ConfigurationFiles``` folders. In this case you would need to provide the path to the file using ```config``` parameter, for example:
```matlab
...
- 'Good' units should have a relatively high firing rate. At minimum they should have firing rates of 360 spikes per hour (0.1 Hz firing frequency). The firing rate criterion over-rides all other criteria. If a unit has a low firing rate, its autocorrelogram is meaningless. However, you can mark them as 'good' temporarily. You would then come back to them to check if they could be merged during the second pass through the data. If they cannot be merged, they should be discarded (marked as unsorted).
...
You would mark 'unsure' clusters initially as 'good'. After splitting (cleaning up) or merging, they should remain marked as 'good' or should be marked as unsorted.

(protocols-spikesorting-split)=
### Split Clusters
You should ideally be splitting clusters as part of the spikesorting Step 3. At this stage you will only be focusing on clusters that were manually marked as 'good' appearing green in the CluserView window (do not confuse with KSLabel 'good'). To display only 'good' clusters type ```group=="good"``` in the filter box located at the top of the CluserView window. This action should filter out any unsorted and 'noise' clusters from the ClusterView window.
...
- You can further clean up by isolating outlying large amplitude events in the AmplitudeView window like you do when you try to split 'unsure' clusters. The large amplitude events may be noise. However, merge them back, if they are too similar to the main cluster spikes.
- Reclustering is useful when you have a unit that has a lightly contaminated refractory period and you want to clean it up. Use the action ```Edit >> Recluster PCAs``` (```Alt+K```). The resulting clusters may be different units or one of the culsters may contain noise. If the new clusers do not look significantly different, merge them back together.

(protocols-spikesorting-merge)=
### Merge Clusters
You should ideally be merging clusters as part of the spikesorting Step 3. At this stage you will only be focusing on clusters that were manually marked as 'good' appearing green in the CluserView window (do not confuse with KSLabel 'good'). To display only 'good' clusters type ```group=="good"``` in the filter box located at the top of the ClusterView window. This action should filter out any unsorted and 'noise' clusters from the ClusterView window.
